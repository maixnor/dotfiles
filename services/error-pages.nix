...
in
{
  ...
  systemd.tmpfiles.rules = [
    ...
    "L+ ${errorPageDir}/index.html - - - - ${pkgs.writeText "404.html" htmlContent}"
  ];
  ...
  environment.etc."traefik/errors.yml" = lib.mkIf cfg.enable {
    text = ''
      http:
        routers:
          # Handle the actual 404 page on the main domain
          error-page-host:
            rule: "Host(`maixnor.com`) && Path(`/404`)"
            priority: 100
          ...
          # Catch unknown subdomains and redirect to HTTPS 404
          catch-all:
            rule: "HostRegexp(`.+.maixnor.com`)"
            priority: 1
            middlewares:
              - "redirect-to-https-404"
            service: "noop"
            entryPoints:
              - "web"
              - "websecure"
        ...
        middlewares:
          ...
          redirect-to-https-404:
            redirectRegex:
              regex: "^https?://.+.maixnor.com/.*$"
              replacement: "https://maixnor.com/404"
              permanent: false
