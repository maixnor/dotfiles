{ ... }:

let
  # YOUR USER KEYS (For SSH Login)
  # Run on your local machine: cat ~/.ssh/id_rsa.pub (or id_ed25519.pub)
  users = {
    maixnor = [
      # bierzelt
      "ssh-rsa  maixnor@bierzelt"
      # bierbasis
      "ssh-rsa  maixnor@Bierbasis"
      # wieselburg
      "ssh-rsa  maixnor@wieselburg"
    ];
  };

  # HOST KEYS (For Agenix Decryption)
  # Run on EACH host: cat /etc/ssh/ssh_host_ed25519_key.pub
  hosts = {
    wieselburg = "COMMAND: ssh maixnor@wieselburg 'cat /etc/ssh/ssh_host_ed25519_key.pub'";
    bierzelt   = "COMMAND: ssh maixnor@bierzelt 'cat /etc/ssh/ssh_host_ed25519_key.pub'";
    bierbasis  = "COMMAND: ssh maixnor@bierbasis 'cat /etc/ssh/ssh_host_ed25519_key.pub'";
  };
in
{
  inherit users hosts;
}